...
/** Sort by room label (name); numeric segments order naturally (e.g. Room 2 before Room 10). */
    ...
    try {
      ...
      const scoped =
        schoolId != null && schoolId !== ''
          ? list.filter((cr) => sameGuid(cr.schoolId, schoolId))
          : list;
      const sorted = [...scoped].sort(sortByRoomNumber);
  ...
  const applyFilters = () => {
    ...
    if (filters.classroomName) {
      filtered = filtered.filter(classRoom =>
        classRoom.name?.toLowerCase().includes(filters.classroomName.toLowerCase())
      );
    }

    if (filters.isActive !== '') {
      const isActive = filters.isActive === 'true';
      filtered = filtered.filter(classRoom => classRoom.isActive === isActive);
    }
  ...
  const handleDelete = async (id, name) => {
    if (window.confirm(`Are you sure you want to delete "${name}"?`)) {
      try {
        ...
        setFilteredClassRooms(filteredClassRooms.filter(cr => cr.id !== id));
      } catch (err) {
      ...
      <div className="card mb-4">
        ...
        <div className="card-body">
          <div className="row g-2 align-items-end">
            <div className="col">
              <label className="form-label small">Classroom Name</label>
              <input
                type="text"
                className="form-control form-control-sm"
                placeholder="Search classroom..."
                value={filters.classroomName}
                onChange={(e) => handleFilterChange('classroomName', e.target.value)}
              />
            </div>
            <div className="col">
      ...
      <div className="card">
        <div className="card-header d-flex justify-content-between align-items-center">
          ...
          <span className="badge bg-secondary">
            Showing {paginatedClassRooms.length} of {filteredClassRooms.length} classrooms
          </span>
        </div>
        <div className="card-body">
          {filteredClassRooms.length === 0 ? (
            <div className="text-center py-4">
              <i className="bi bi-door-open display-4 text-muted"></i>
              <p className="text-muted mt-3">No classrooms found</p>
              <Link to="/classrooms/create" className="btn btn-outline-primary">
                ...
                <table className="table table-striped table-hover">
                  ...
                  <tbody>
                    {paginatedClassRooms.map((classRoom) => (
                      <tr key={classRoom.id}>
                        <td>
                          <Link to={`/classrooms/${classRoom.id}`} className="text-decoration-none">
                            {classRoom.name || 'N/A'}
                          </Link>
                        </td>
                        <td>
