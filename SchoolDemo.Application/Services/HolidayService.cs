...
public class HolidayService : IHolidayService
{
	...
	public HolidayService(IHolidayRepository repository, IHttpContextAccessor httpContextAccessor)
	{
	...
	public async Task<HolidayResponse?> UpdateAsync(Guid id, HolidayRequest request)
	{
		...
		// Set ModifiedBy to the currently authenticated user if available; otherwise preserve existing value
		string? userIdClaim = _httpContextAccessor.HttpContext?.User?.FindFirst(ClaimTypes.NameIdentifier)?.Value
							  ?? _httpContextAccessor.HttpContext?.User?.FindFirst("sub")?.Value;
		if (Guid.TryParse(userIdClaim, out var currentUserId))
		{
			existingEntity.ModifiedBy = currentUserId;
		}
