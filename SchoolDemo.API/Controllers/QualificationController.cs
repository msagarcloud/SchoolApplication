...
public class QualificationController : ControllerBase
{
    ...
    [HttpPost]
    public async Task<IActionResult> Create([FromBody] QualificationRequest request)
    {
        ...
        {
            return StatusCode(500, $"An error occurred while creating qualification: {ex.Message}");
        }
    }

    [HttpPut("{id}")]
    public async Task<IActionResult> Update(Guid id, [FromBody] QualificationRequest request)
    {
        if (!ModelState.IsValid)
        {
            return BadRequest(ModelState);
