...
const CountryForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditing) {
        await countryService.update(id, formData);
      } else {
      ...
      setError(err.message || `Failed to ${isEditing ? 'update' : 'create'} country`);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="container-fluid">
      ...
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h2>{isEditing ? 'Edit Country' : 'Create New Country'}</h2>
      ...
      <div className="card">
        <div className="card-header">
          <h5 className="mb-0">
            {isEditing ? 'Country Information' : 'New Country Details'}
          </h5>
        </div>
