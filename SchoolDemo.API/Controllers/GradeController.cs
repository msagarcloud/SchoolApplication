...
public class GradeController : ControllerBase
{
    ...
    public async Task<IActionResult> Create([FromBody] GradeRequest request)
    {
        ...
        {
            return StatusCode(500, $"An error occurred while creating grade: {ex.Message}");
        }
    }

    [HttpPut("{id}")]
    public async Task<IActionResult> Update(Guid id, [FromBody] GradeRequest request)
    {
        if (!ModelState.IsValid)
        {
            return BadRequest(ModelState);
    ...
    public async Task<IActionResult> Delete(Guid id)
    {
        ...
        {
            return StatusCode(500, $"An error occurred while deleting grade: {ex.Message}");
        }
    }
}