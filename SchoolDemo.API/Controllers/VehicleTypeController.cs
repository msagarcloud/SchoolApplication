...
public class VehicleTypeController : ControllerBase
{
    ...
    public async Task<IActionResult> Create([FromBody] VehicleTypeRequest request)
    {
        ...
        {
            return StatusCode(500, $"An error occurred while creating vehicle type: {ex.Message}");
        }
    }

    [HttpPut("{id}")]
    public async Task<IActionResult> Update(Guid id, [FromBody] VehicleTypeRequest request)
    {
        if (!ModelState.IsValid)
        {
            return BadRequest(ModelState);
        ...
        {
            return StatusCode(500, $"An error occurred while updating vehicle type: {ex.Message}");
        }
    }
    ...
    public async Task<IActionResult> Delete(Guid id)
    {
        ...
        {
            return StatusCode(500, $"An error occurred while deleting vehicle type: {ex.Message}");
        }
    }
}