...
public class EnquiryController : ControllerBase
{
    ...
    public async Task<IActionResult> Create([FromBody] EnquiryRequest request)
    {
        ...
        {
            return StatusCode(500, $"An error occurred while creating enquiry: {ex.Message}");
        }
    }

    [HttpPut("{id}")]
    public async Task<IActionResult> Update(Guid id, [FromBody] EnquiryRequest request)
    {
        if (!ModelState.IsValid)
        {
            return BadRequest(ModelState);
        ...
        {
            return StatusCode(500, $"An error occurred while updating enquiry: {ex.Message}");
        }
    }
    ...
    public async Task<IActionResult> Delete(Guid id)
    {
        ...
        {
            return StatusCode(500, $"An error occurred while deleting enquiry: {ex.Message}");
        }
    }
}